...
import type { LocalPayload, TipoLocal } from "../../services/locadoresService";
...
function buildDefaultHorarios(
  ...
  const fromInitial =
    (initial?.horarios as HorarioDia[] | undefined) ?? undefined;
...
export default function LocalDialog({
  ...
  const handleChange = <K extends keyof LocalFormValues>(
    key: K,
    val: LocalFormValues[K]
  ) => {
  ...
  const handleRemoveSelectedPhoto = () => {
    ...
    setValues((prev) => {
      ...
      if (selectedPhotoIndex < existingCount) {
        ...
        const newAllLen =
          newFotos.length + (prev.novasFotosPreview?.length ?? 0);
        const newIndex = Math.max(
          0,
          Math.min(selectedPhotoIndex, newAllLen - 1)
        );
      ...
      const newNovasPreview = prev.novasFotosPreview.filter(
        (_, i) => i !== offset
      );
  ...
  const handleSave = async () => {
    try {
      ...
      // monte endereco final
      const enderecoFinal = [values.endereco].filter(Boolean).join(" • ");

      const payload: LocalPayload = {
        nome: values.nome,
        descricao: values.descricao,
        cep: (values as any).cep ?? (values as any).cepts ?? "",
      ...
      if (mode === "create") {
        if (typeof onCreate !== "function")
          throw new Error("onCreate não foi passado");
        await onCreate(payload, { novasFotos: values.novasFotos ?? [] });
      } else {
        if (typeof onUpdate !== "function")
          throw new Error("onUpdate não foi passado");
        await onUpdate(values.id!, payload, {
          novasFotos: values.novasFotos ?? [],
        });
      }

      // revoga previews blob
      values.novasFotosPreview?.forEach((u) => {
        if (u.startsWith("blob:")) URL.revokeObjectURL(u);
      });
              ...
              <Stack direction={"row"} spacing={2}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleChange("tipoLocal", e.target.value as TipoLocal)
                    }
                  >
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    handleChange("precoHora", Number(e.target.value))
                  }
                  fullWidth
                    ...
                    >
                      ...
                      >
                        <Select
                          ...
                          onChange={(e) =>
                            setHorario(d.id, {
                              aberto: e.target.value === "aberto",
                            })
                          }
                          renderValue={(value) => {
                            if (!isMobile)
                              return value === "aberto" ? "Aberto" : "Fechado";
                      ...
                      <TextField
                        ...
                        value={h.inicio}
                        onChange={(e) =>
                          setHorario(d.id, { inicio: e.target.value })
                        }
                        disabled={!h.aberto}
                        sx={{ width: 140 }}
                      />
                      ...
                      <TextField
                        ...
                        value={h.fim}
                        onChange={(e) =>
                          setHorario(d.id, { fim: e.target.value })
                        }
                        disabled={!h.aberto}
                        sx={{ width: 140 }}
                      />
